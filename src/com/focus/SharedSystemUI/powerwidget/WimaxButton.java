...
public class WimaxButton extends PowerButton {
	...
	private static final class WimaxStateTracker extends StateTracker {
		...
		public void onActualStateChange(Context context, Intent intent) {
			...
			if (action
					.equals(WimaxManagerConstants.NET_4G_STATE_CHANGED_ACTION)) {
				wimaxState = intent.getIntExtra(
						WimaxManagerConstants.EXTRA_4G_STATE,
						WimaxManagerConstants.NET_4G_STATE_UNKNOWN);
			} else if (action
					.equals(WimaxManagerConstants.WIMAX_ENABLED_CHANGED_ACTION)) {
				wimaxState = intent.getIntExtra(
						WimaxManagerConstants.CURRENT_WIMAX_ENABLED_STATE,
						WimaxManagerConstants.NET_4G_STATE_UNKNOWN);
			} else {
				return;
			}
		...
		/**
		 * Converts Wimax4GManager's state values into our WiMAX-common state
		 * values. Also compatible with WimaxController state values.
		 */
	...
	protected boolean handleLongClick(Context context) {
		...
		intent.setClassName("com.android.settings.wimax",
				"com.android.settings.wimax.WimaxSettings");
